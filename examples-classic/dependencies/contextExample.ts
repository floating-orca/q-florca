...
const plusOne = async (number: number, context: PluginContext) => {
  console.log(`plusOne(${number}): Calculating...`);
  await new Promise((resolve) => setTimeout(resolve, 10000 / (number + 1)));
  const result = await context.run("increment", number);
...
export default async (
  ...
  const plusOnes = numbers.map((number) =>
    plusOne(number, requestBody.context)
  );
